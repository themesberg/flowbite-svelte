...
test("index page has expected h1", async ({ page }) => {
  await expect(page.getByRole("heading", { name: "Flowbite Svelte", level: 1 })).toBeVisible();
});

test("index page has expected meta title", async ({ page }) => {
  await expect(page).toHaveTitle("Flowbite Svelte");
});
...
test("index page has expected meta og", async ({ page }) => {
  ...
  await expect(metaOgTitle).toHaveAttribute("content", "Flowbite Svelte");
  ...
  await expect(metaOgImage).toHaveAttribute("content", "https://open-graph-vercel.vercel.app/api/flowbite-svelte");
});

test("index page has expected meta twitter", async ({ page }) => {
  const metaTwitterTitle = page.locator('meta[name="twitter:title"]');
  await expect(metaTwitterTitle).toHaveAttribute("content", "Flowbite Svelte");
  const metaTwitterDescription = page.locator('meta[name="twitter:description"]');
  await expect(metaTwitterDescription).toHaveAttribute("content", "A UI library for Svelte 5 Runes.");
  const metaTwitterImage = page.locator('meta[name="twitter:image"]');
  await expect(metaTwitterImage).toHaveAttribute("content", "https://open-graph-vercel.vercel.app/api/flowbite-svelte");
});
