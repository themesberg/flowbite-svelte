...
test("alert page has expected meta title", async ({ page }) => {
  await expect(page).toHaveTitle("Alert - Flowbite Svelte");
});
...
test("alert page has expected meta og", async ({ page }) => {
  ...
  await expect(metaOgTitle).toHaveAttribute("content", "Alert - Flowbite Svelte");
  ...
  await expect(metaOgImage).toHaveAttribute("content", "https://open-graph-vercel.vercel.app/api/flowbite-svelte?title=Alert");
});

test("alert page has expected meta twitter", async ({ page }) => {
  const metaTwitterTitle = page.locator('meta[name="twitter:title"]');
  await expect(metaTwitterTitle).toHaveAttribute("content", "Alert - Flowbite Svelte");
  const metaTwitterDescription = page.locator('meta[name="twitter:description"]');
  await expect(metaTwitterDescription).toHaveAttribute("content", "Alert component for Svelte 5 Runes.");
  const metaTwitterImage = page.locator('meta[name="twitter:image"]');
  await expect(metaTwitterImage).toHaveAttribute("content", "https://open-graph-vercel.vercel.app/api/flowbite-svelte?title=Alert");
});
