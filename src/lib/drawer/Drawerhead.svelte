...
<div class={base({ class: clsx((theme as DrawerHeadTheme)?.base, className) })}>
  ...
  {#if closeIcon}
    {@render closeIcon()}
  {:else}
    <button type="button" {...restProps} class={button({ class: clsx((theme as DrawerHeadTheme)?.button, styling.button) })}>
      <svg class={svg({ class: clsx((theme as DrawerHeadTheme)?.svg, styling.svg) })} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
...
## Type
[DrawerheadProps](https://github.com/themesberg/flowbite-svelte/blob/main/src/lib/types.ts#L564)
## Props
@prop closeIcon
@prop children
@prop buttonClass
