...
<div class={root({ class: clsx((theme as TableSearchTheme)?.root, styling.root) })}>
  <div class={inner({ class: clsx((theme as TableSearchTheme)?.inner, styling.inner) })}>
    <label for="table-search" class="sr-only">Search</label>
    <div class={search({ class: clsx((theme as TableSearchTheme)?.search, styling.search) })}>
      <div class={svgDiv({ class: clsx((theme as TableSearchTheme)?.svgDiv, styling.svgDiv) })}>
        <svg class={svg({ class: clsx((theme as TableSearchTheme)?.svg, styling.svg) })} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd" />
        </svg>
      </div>
      <input bind:value={inputValue} type="text" id="table-search" class={input({ class: clsx((theme as TableSearchTheme)?.input, styling.input) })} {placeholder} />
    </div>
    {#if header}
      {@render header()}
    {/if}
