<script lang="ts">
    ...
    let {children, arrowup, arrowdown, icon, btnClass, label, spanClass, ulClass, transitionType = 'slide', transitionParams = {}, ...attributes} = $props<Props>()

    let btnCls = twMerge('flex items-center p-2 w-full text-base font-normal text-gray-900 rounded-lg transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700', btnClass)
