<script lang="ts">
  ...
  let { children, outline = false, pill = false, color = 'blue', shadow = false, ...attributes } = $props<Props>();
  ...
  $effect(()=>{gradientOutlineClass = twMerge(
  ...
  $effect(()=>{ divClass = twMerge(outline && 'p-0.5', gradientClasses[color], shadow && 'shadow-lg', shadow && coloredShadowClasses[color], group ? (pill && 'first:rounded-s-full last:rounded-e-full') || 'first:rounded-s-lg last:rounded-e-lg' : (pill && 'rounded-full') || 'rounded-lg')});
</script>
...
{#if outline}
  <div class={divClass}>
    ...
    <Button {...attributes} color="none" class={gradientOutlineClass} on:click on:change on:keydown on:keyup on:mouseenter on:mouseleave>
      {@render children()}
      </Button>
  </div>
{:else}
  <Button {...attributes} color="none" class={divClass} on:click on:change on:keydown on:keyup on:touchstart on:touchend on:touchcancel on:mouseenter on:mouseleave>
    {@render children()}
  </Button>
{/if}