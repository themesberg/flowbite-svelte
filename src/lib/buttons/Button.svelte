<script lang="ts">
  ...
  let { children, pill = false, outline = false, size = group ? 'sm' : 'md', href = undefined, target = undefined, rel = undefined, type = 'button', color = group ? (outline ? 'dark' : 'alternative') : 'primary', shadow = false, tag = 'button', checked = undefined, disabled = false, name = undefined, class: btnclass, onclick, ...restProps }: Props = $props();
