<script lang="ts">
  ...
  let { children, pill = false, outline = false, size = group ? 'sm' : 'md', href = undefined, type = 'button', color = group ? (outline ? 'dark' : 'alternative') : 'primary', shadow = false, tag = 'button', checked = undefined, disabled = false, class: classname, ...attributes } = $props<Props>()
