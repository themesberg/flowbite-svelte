<script lang="ts">
  ...
  import { type ProgressStepperProps, type ProgressStep } from '$lib';

  let { children, steps = [], class: className }: ProgressStepperProps = $props();
...
<ol class={base}>
  ...
  {:else if steps}
    {#each steps as step, index}
      <li class={progressstepperitem({ 
        status: step.status, 
        isLast: index === steps.length - 1 
      })}>
        <span class={progresssteppercircle({ status: step.status })}>
          {#if step.status === 'completed'}
            {#if step.icon}
              <step.icon class={step.iconClass || "w-5 h-5 lg:w-6 lg:h-6"} />
            {:else}
