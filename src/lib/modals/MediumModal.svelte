<script>
  ...
  export let btn1 = "I accept";
  export let btn2 = "Decline";
...
>
  <div class="relative px-4 w-full max-w-2xl h-full md:h-auto">
    ...
    <div class="relative bg-white rounded-lg shadow dark:bg-{textColor}-700">
      <!-- Modal header -->
      <div
        class="flex justify-between items-start p-5 rounded-t border-b dark:border-{textColor}-600"
      >
        <h3
          class="text-xl font-semibold text-{textColor}-900 lg:text-2xl dark:text-white"
        >
          {title}
        </h3>
        <button
      ...
      >
        ...
        {#if btn2}
          <button
            ...
            on:click={() => closeMediumModal()}>{btn2}</button
          >
        {/if}
      </div>
    </div>
