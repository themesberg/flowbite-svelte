<script lang="ts">
  ...
  export let unWrappedClass: string = 'p-2.5 text-sm focus:ring-primary-500 focus:border-primary-500 dark:focus:ring-primary-500 dark:focus:border-primary-500 disabled:cursor-not-allowed disabled:opacity-50';
  export let innerWrappedClass: string = 'py-2 px-4 bg-white dark:bg-gray-800';
  let wrapped: boolean;
  $: wrapped = $$slots.header || $$slots.footer;

  let wrapperClass: string;
  $: wrapperClass = twMerge('w-full rounded-lg bg-gray-50', background ? 'dark:bg-gray-600' : 'dark:bg-gray-700', 'text-gray-900 dark:placeholder-gray-400 dark:text-white ', 'border border-gray-300', background ? 'dark:border-gray-500' : 'dark:border-gray-600', $$props.class);

  let textareaClass: string;
  $: textareaClass = wrapped ? wrappedClass : twMerge(wrapperClass, unWrappedClass);

  const headerClass = (header: boolean) => twMerge(header ? 'border-b' : 'border-t', 'py-2 px-3 border-gray-300', background ? 'dark:border-gray-500' : 'dark:border-gray-600');
...
@prop export let unWrappedClass: string = 'p-2.5 text-sm focus:ring-primary-500 focus:border-primary-500 dark:focus:ring-primary-500 dark:focus:border-primary-500 disabled:cursor-not-allowed disabled:opacity-50';
