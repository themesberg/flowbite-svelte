...
<div class="mb-5 flex items-center">
  {#if headerLabel}
    {#if headerLabel.desc1}
      <p class={desc1({ class: clsx((theme as ScoreRatingTheme)?.desc1, classes?.desc1) })}>{headerLabel.desc1}</p>
    {/if}
    {#if headerLabel.desc2}
      <p class={desc2({ class: clsx((theme as ScoreRatingTheme)?.desc2, classes?.desc2) })}>{headerLabel.desc2}</p>
    {/if}
    {#if headerLabel.desc3}
      <span class={desc3span({ class: clsx((theme as ScoreRatingTheme)?.desc3span, classes?.desc3span) })}></span>
      <p class={desc3p({ class: clsx((theme as ScoreRatingTheme)?.desc3p, classes?.desc3p) })}>{headerLabel.desc3}</p>
    {/if}
    {#if headerLabel.link}
      <a href={headerLabel.link.url} class={link({ class: clsx((theme as ScoreRatingTheme)?.link, classes?.link) })}>{headerLabel.link.label}</a>
    {/if}
  {/if}
</div>
        ...
        <dl>
          ...
          <dd class="mb-3 flex items-center">
            <div class="me-2 h-2.5 w-full rounded-sm bg-gray-200 dark:bg-gray-700">
              <div class={bar({ class: clsx((theme as ScoreRatingTheme)?.bar, classes?.bar) })} style="width: {rating * 10}%"></div>
            </div>
