<script lang="ts">
  ...
  {#if labelOutside}
    <div {...attributes} class={twMerge('flex justify-between mb-1', labelOutsideClass)}>
  ...
  <div class={twMerge(divCls, size, divClass)}>
    {#if labelInside}
      <div class={twJoin(labelInsideCls, barColors[color], size, labelInsideClass)} style="width: {$_progress}%">
