export const handleTabs = () => {
  document.querySelectorAll('[data-tabs-toggle]').forEach(function (tabsToggleEl) {
    ...
    let activeTabToggleEl = null;
    let activeTabContentEl = null;
    tabsToggleElements.forEach(function (tabToggleEl) {
      tabToggleEl.addEventListener('click', function (event) {
        let tabToggleEl = event.target;
        let tabTargetSelector = tabToggleEl.getAttribute('data-tabs-target');
        let tabContentEl = document.querySelector(tabTargetSelector); // don't do anything if it's already active
