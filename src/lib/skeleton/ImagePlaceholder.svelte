...
<div
  role="status"
  {...restProps}
  class={base({ class: clsx(className, (theme as ImagePlaceholderTheme)?.base) })}
>
  <div
    class={image({ class: clsx((theme as ImagePlaceholderTheme)?.image) })}
  >
    <svg
      width="48"
      height="48"
      class={svg({ class: clsx((theme as ImagePlaceholderTheme)?.svg) })}
  ...
  {#if !imgOnly}
    <div
      class={content({ class: clsx((theme as ImagePlaceholderTheme)?.content) })}
    >
      <div
        class={line({
