...
<h1 class="text-3xl my-4">API check</h1>

<h2 class="text-2xl my-4">Components</h2>
...
<List class="space-y-1 text-gray-500 dark:text-gray-400 my-8">
		...
		<Li icon>
			...
			{#if problems}
				<span class="ml-4 text-red-500">( {problems})</span>{/if}{#if notes}<span
					class="ml-4 text-yellow-500">(Todos: {notes})</span
				>
			{/if}
		</Li>
	{/each}
...
<h2 class="text-2xl my-4">Forms</h2>
<List class="space-y-1 text-gray-500 dark:text-gray-400">
...
<h2 class="text-2xl my-4">Typography</h2>
<List class="space-y-1 text-gray-500 dark:text-gray-400">
