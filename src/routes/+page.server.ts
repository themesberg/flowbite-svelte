...
export const load: PageServerLoad = async ({ fetch }) => {
  try {
    const githubAPI = "https://api.github.com/repos/themesberg/flowbite-svelte-next";
    ...
    const npm = await fetch(`https://api.npmjs.org/downloads/point/2020-01-01:${today}/flowbite-svelte-next`).then(json);
    const discord = await fetch("https://discord.com/api/v9/invites/4eeurUVvTy?with_counts=true&with_expiration=true").then(json);

    return { posts, builders, contributors, github, npm, discord };
  } catch (error) {
