<script lang="ts">
  ...
  type ToastColor = "primary" | "green" | "red" | "yellow" | "blue";
...
<div class="relative min-h-screen p-8">
  <div class="absolute top-4 right-4 z-50 space-y-3" style="position: absolute; top: 1rem; right: 1rem;">
    {#each toasts as toast (toast.id)}
      <div onclose={handleClose(toast.id)}>
        <Toast color={toast.color} dismissable={true} transition={fly} params={{ x: 400, duration: 300 }} class="w-64">
          {toast.message}
        </Toast>
      </div>
    {/each}
