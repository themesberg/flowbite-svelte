<script lang="ts">
  import { ThemeProvider, Button, Card, Alert, Skeleton } from "flowbite-svelte";
    ...
    <div>
      ...
      <div class="flex flex-wrap gap-2">
        {#each colors as color}
          <button class="rounded px-4 py-2 capitalize transition-colors {selectedColor === color ? colorClasses[color] : 'bg-gray-200 hover:bg-gray-300'}" onclick={() => (selectedColor = color)}>
            {color}
          </button>
        {/each}
      </div>
