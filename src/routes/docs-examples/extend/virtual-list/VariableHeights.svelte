...
<VirtualList {items} minItemHeight={100} {getItemHeight} height={400}>
  {#snippet children(item, index)}
    {@const typedItem = item as Item}
    <div 
      class="border-b p-3 hover:bg-gray-50 dark:hover:bg-gray-800"
      style="height:{getItemHeight(typedItem)}px"
    >
      <div class="font-semibold text-gray-900 dark:text-white">{typedItem.title}</div>
      <div class="text-sm text-gray-600 dark:text-gray-400 mt-1">{typedItem.description}</div>
      <div class="text-xs text-gray-500 mt-1">Height: {getItemHeight(typedItem)}px</div>
    </div>
  {/snippet}
</VirtualList>