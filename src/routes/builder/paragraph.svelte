...
<CodeWrapper>
	...
	<div class="mb-4 flex flex-wrap space-x-4">
		...
		{#each sizes as size}
			<Radio class="w-12 my-1" name="p_size" bind:group={pSize} value={size}>{size}</Radio>
		{/each}
	</div>
	<div class="mb-4 flex flex-wrap space-x-4">
		<Label class="mb-4 w-full font-bold">Weight</Label>
		{#each weights as weight}
			<Radio class="w-20 my-1" name="p_weight" bind:group={pWeight} value={weight}
				>{weight}</Radio
			>
		{/each}
	</div>
	<div class="mb-4 flex flex-wrap space-x-2">
		...
		{#each spaces as space}
			<Radio class="w-20 my-1" name="p_space" bind:group={pSpace} value={space}>{space}</Radio>
		{/each}
	</div>
	<div class="mb-4 flex flex-wrap space-x-4">
		<Label class="mb-4 w-full font-bold">Height(Leading)</Label>
		{#each heights as height}
			<Radio class="w-16 my-1" name="p_height" bind:group={pHeight} value={height}
				>{height}</Radio
			>
		{/each}
	</div>
	...
	<div class="mb-4 flex flex-wrap space-x-4">
		...
		{#each whitespaces as whitespace}
			<Radio class="w-16 my-1" name="p_whitespace" bind:group={pWhitespace} value={whitespace}
				>{whitespace}</Radio
			>
		{/each}
	</div>
