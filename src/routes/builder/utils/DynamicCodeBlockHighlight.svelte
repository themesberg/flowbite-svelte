  ...
  <div class="relative">
    <div class="overflow-hidden {showExpandButton ? 'pb-8' : ''}" class:max-h-56={!expand} tabindex="-1">
      ...
      {:else if processedCode}
        <div class="highlight">
          <pre class="language-svelte !-mt-2 mb-0 !rounded-none">{processedCode}</pre>
        </div>
      {:else}
        no code is provided
      {/if}
    ...
    {#if showExpandButton}
      <button
        ...
        type="button"
        class="hover:text-fg-brand absolute start-0 bottom-0 w-full border-t border-default-strong bg-neutral-tertiary-medium px-5 py-2.5 text-sm font-medium text-body hover:bg-neutral-tertiary-medium"
      >
