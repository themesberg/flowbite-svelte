...
export const load: PageLoad = async ({ params }) => {
  ...
  if (!pagesFiles[filePath]) {
    throw error(404, {
      message: `Page documentation "${slug}" not found`
    });
  }
  
  // Safely import the file
  const post = await pagesFiles[filePath]() as MarkdownModule;
  
  // Validate metadata exists
  if (!post.metadata || typeof post.metadata !== 'object') {
    throw error(500, {
      message: `Invalid metadata for page doc: ${slug}`
    });
  }
