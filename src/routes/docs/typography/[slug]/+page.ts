...
export const load: PageLoad = async ({ params }) => {
  ...
  if (!typographyFiles[filePath]) {
    throw error(404, {
      message: `Typography documentation page "${slug}" not found`
    });
  }
  
  // Safely import the file
  const post = await typographyFiles[filePath]() as MarkdownModule;
  
  // Validate metadata exists
  if (!post.metadata || typeof post.metadata !== 'object') {
    throw error(500, {
      message: `Invalid metadata for typography doc: ${slug}`
    });
  }
