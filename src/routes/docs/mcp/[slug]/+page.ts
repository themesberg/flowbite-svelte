...
export const load: PageLoad = async ({ params }) => {
  ...
  if (!mcpFiles[filePath]) {
    throw error(404, {
      message: `MCP documentation page "${slug}" not found`
    });
  }
  
  // Safely import the file
  const post = await mcpFiles[filePath]() as MarkdownModule;
  
  // Validate metadata exists
  if (!post.metadata || typeof post.metadata !== 'object') {
    throw error(500, {
      message: `Invalid metadata for MCP doc: ${slug}`
    });
  }
